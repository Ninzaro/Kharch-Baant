...
function injectNativeSsoClerkKey(publishableKey: string) {
  return {
    name: 'inject-native-sso-clerk-key',
    closeBundle() {
      const file = path.resolve(__dirname, 'dist/native-sso.html');
...
export default defineConfig(({ mode }) => {
    ...
    const clerkPk = env.VITE_CLERK_PUBLISHABLE_KEY || process.env.VITE_CLERK_PUBLISHABLE_KEY || '';
