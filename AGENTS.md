...
Native (Capacitor) unauthenticated launch is `WelcomeScreen` → **Get started** → `AuthScreen`. On Android, **Continue with Google** is `ClerkNativeAuthPlugin` (clerk-android `signInWithOAuth(GOOGLE)` or `signUpWithOAuth(GOOGLE)`, according to the selected Clerk screen) → `getToken()` → Edge `native-bridge` (`verifyToken` + Sign-in Token) → clerk-react `signIn.create({ strategy: 'ticket' })`. Email/password stays on Clerk's `<SignIn />` / `<SignUp />` components. Web/desktop keep normal clerk-react (including Google). Do not add Google hosts to `allowNavigation`. Do not open `clerk.motamaati.in` in a browser tab (FAPI JSON `authorization_invalid`). Account Portal / `native-sso.html` / `kharchbaant://sso-callback` remain in the repo but must not be used by the Android Google button. Full postmortem: [`docs/superpowers/specs/2026-09-06-native-android-google-auth.md`](./docs/superpowers/specs/2026-09-06-native-android-google-auth.md).
