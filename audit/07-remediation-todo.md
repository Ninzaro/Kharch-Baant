...
| high | Anonymous callers retain direct `people` insert privilege. | Live check 2026-09-22: `has_table_privilege('anon','people','INSERT')` was true; `authenticated` already false. Placeholder path is `create_unclaimed_person` (DEFINER, authenticated EXECUTE only). Applied `20260922000000_revoke_anon_people_insert.sql`. Post-apply: anon INSERT false; `POST /rest/v1/people` with anon key returns 401 `42501 permission denied for table people`. | Closed. INSERT RLS still has the unclaimed-row disjunct; harmless without INSERT grant. Anon still has SELECT/UPDATE/DELETE/TRUNCATE on `people` — separate grant cleanup. |
| high | Execute grants do not match the R-20/R-21-era least-privilege intent. | Live 2026-09-22: `i_am_person` and `set_transaction_author` were executable by `anon`; `get_current_user_person_id` by PUBLIC/anon/authenticated. SELECT policies do not call `i_am_person`. Applied `20260922000001_revoke_anon_rpc_execute.sql`. Anon `POST /rpc/i_am_person` → 401 `42501`. `get_invite_preview` / `i_am_member_of` / `i_created_group` / `i_can_see_person` left executable so pre-auth invite + SELECT RLS still evaluate. | Closed. Dead function `get_current_user_person_id` still exists (execute revoked); drop is a separate item. |
