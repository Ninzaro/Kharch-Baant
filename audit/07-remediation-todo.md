...
| high | Anonymous callers retain direct `people` insert privilege. | INSERT revoked in `20260922000000`. Remaining anon table DML revoked in `20260922000003_revoke_anon_table_dml.sql` (SELECT/INSERT/UPDATE/DELETE/TRUNCATE and the rest of GRANT ALL) on the nine public app tables. `authenticated` SELECT/INSERT kept. Anon `GET /people` → 401 `42501`. Invite preview stays on `get_invite_preview`. | Closed. |
| high | Execute grants do not match the R-20/R-21-era least-privilege intent. | Live 2026-09-22: `i_am_person` and `set_transaction_author` were executable by `anon`; `get_current_user_person_id` by PUBLIC/anon/authenticated. SELECT policies do not call `i_am_person`. Applied `20260922000001_revoke_anon_rpc_execute.sql`. Anon `POST /rpc/i_am_person` → 401 `42501`. | Closed. `get_current_user_person_id` later dropped. |
| medium | A-27 RLS helpers callable as RPCs. | Revoking `authenticated` EXECUTE breaks policies (`42501` on `SELECT groups`). Applied `20260922000004_hide_rls_helpers.sql`: real helpers live in `app_private` (EXECUTE kept for `authenticated`). Public stubs have no client EXECUTE. Anon `POST /rpc/i_am_member_of` → 404 `PGRST202`. | Closed. |
| medium | A-26 `debug_auth_check` has no repo source. | Historical body returned JWT sub, role, and `requesting_user_id()`. Dropped by `20260912000004` (recorded on production). `get_current_user_person_id` dropped in `20260922000002`. Neither name exists in any schema. Anon `POST /rpc/debug_auth_check` → 404 `PGRST202`. Not in `database.types.ts`. Remaining live functions are defined in `supabase/migrations` or `supabase-schema.sql` (`update_updated_at_column`). | Closed. No new SQL. |
| high | R-21 native-bridge has no audience check. | Repo `9ec2ccd` rejects a verified token when `azp` is non-empty (web session JWTs) and accepts the clerk-android shape where `azp` is missing. Rate limit runs before `verifyToken`. Production `native-bridge` v10 (2026-09-19) contains `azp_present`. `authorizedParties` is not set: a real native token has no `azp`, so an allowlist would reject Android sign-in. | Closed at containment. Shared-secret second factor was not added. |
| high | R-23 / S-11 public Realtime topics. | Broadcast `tx` is already gone. Clients use `private: true` on the five `postgres_changes` topics. `20260923000000_realtime_private_topics.sql` lets `authenticated` SELECT `realtime.messages` only for those topics. No INSERT policy, so clients cannot broadcast. | Policy applied. Dashboard step still required: Realtime Settings → turn **Allow public access** off. Until that is off, an anon key can still join. |
| high | R-02 verified-email claim. | `20260923000001_verified_email_claim.sql`: placeholder claim uses JWT `email` only when `email_verified` is true. A different `p_email` raises `42501` and leaves the placeholder unclaimed. `claim_person_by_email` stays revoked. | SQL live. Clerk session token still needs `email` and `email_verified` claims or claiming stays off. |
| medium | A-17 Edge rate limits are per-isolate Maps. | `app_private.consume_budget` is the shared counter. Same caps: email 20/min, suggest-tag 30/min, native-bridge 10/min. A rolled-back probe allowed 2 and blocked the 3rd. Functions redeployed. PostgREST RPCs are still unlimited. | Edge path closed. Item 7 audit history not started. |
| high | R-14 consent before adding a claimed user. | Direct insert of a claimed person was already rejected. Add Member now sends a one-use email invite (`inviteClaimedMember`). `20260923000003_consent_before_add.sql`: `accept_group_invite` returns an error when pending `email_invites` do not match the signed-in email or verified JWT email. Placeholders can still be added directly. | SQL live. Modal was not clicked through in a browser. |
| medium | Obsolete Supabase-Auth helper survives outside the canonical ledger. | `get_current_user_person_id()` used `auth.uid()` / `people.auth_user_id`. Live policies and other public functions did not reference it. EXECUTE revoked in `20260922000001`; dropped in `20260922000002_drop_get_current_user_person_id.sql`. | Closed. |
